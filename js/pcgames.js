const FUZZYSCORE_THRESHOLD = -1000;
...
function filterItems() {
    ...
    const filter_tags = filter_text.startsWith('#');
    const include_played = $('#checkbox-status-played').prop('checked');
    const include_unplayed = $('#checkbox-status-unplayed').prop('checked');
    const platform_steam = $('#checkbox-platform-steam').prop('checked');
    const platform_epic = $('#checkbox-platform-epic').prop('checked');
    const platform_ubisoft = $('#checkbox-platform-ubisoft').prop('checked');
    const platform_origin = $('#checkbox-platform-origin').prop('checked');
    const platform_gog = $('#checkbox-platform-gog').prop('checked');
    const platform_retail = $('#checkbox-platform-retail').prop('checked');
    const platform_console = $('#checkbox-platform-console').prop('checked');
    let num_items = 0;
    $('#content').children().each(function(index, item) {
        const played = $(item).attr('played');
        const matches_status = include_played && played == 'true' || include_unplayed && played == 'false';
        if (!matches_status) {
            $(item).fadeOut(FADE_DURATION, $(item).hide);
            return true;
        }
        const matches_platform = platform_steam && $(item).find(".steam").length || platform_epic && $(item).find(".epic").length || platform_ubisoft && $(item).find(".ubisoft").length || platform_origin && $(item).find(".origin").length || platform_gog && $(item).find(".gog").length || platform_retail && $(item).find(".retail").length || platform_console && $(item).find(".playstation, .playstation-2").length;
        if (!matches_platform) {
            $(item).fadeOut(FADE_DURATION, $(item).hide);
            return true;
        }
        ...
        if (filter_text && !filter_tags) {
            const result = fuzzysort.single(filter_text, $(item).attr('game-title').toLowerCase());
            if (!result || result.score < FUZZYSCORE_THRESHOLD) {
                $(item).fadeOut(FADE_DURATION, $(item).hide);
                return true;
            }
        }
        $(item).fadeIn(FADE_DURATION);
        num_items++;
    });
    if (filter_text) {
        $('#filter-icon').removeClass('bx-search-alt-2');
        $('#filter-icon').addClass('bx-x');
        $('#sidebar-title').text(`${num_items} Games`);
    } else {
...
$(function() {
    ...
    let title_idx;
    let rating_idx;
    fetch('pcgames.json').then(response => response.json()).then(data => {
        for (const game of data['games']) {
            const title = game['title'];
            let release;
            const platforms = game['platform'];
            const img = game.hasOwnProperty('app_id') ? game['app_id'] + 'p.webp' : game['img'];
            const rating = game['rating'];
            const played = rating !== null;
            let tooltip_html = title;
            if (typeof game['release'] === 'object') {
                for (const [key, value] of Object.entries(game['release'])) {
                    tooltip_html += `<br>${key}: ${parseDate(value)}`;
                }
                release = Object.values(game['release'])[0];
            } else {
                release = game['release'];
                tooltip_html += '<br>' + parseDate(release);
            }
            const release_year = parseInt(release.slice(0, 4));
            if (played) {
                tooltip_html += '<br>';
                for (let j = 0; j < Math.floor(rating); j++) {
                    tooltip_html += '<i class=\'bx bxs-star\'></i>';
                }
                if (!Number.isInteger(rating)) {
                    tooltip_html += '<i class=\'bx bxs-star-half\'></i>';
                }
                for (let j = Math.ceil(rating); j < 5; j++) {
                    tooltip_html += '<i class=\'bx bx-star\'></i>';
                }
                years_played.push(release_year);
            } else {
                years_unplayed.push(release_year);
            }
            let img_html = `<img loading="lazy" src="img/${img}" alt="${title}" title="${tooltip_html}">`;
            if (game['link']) {
                img_html = `<a href="${game['link']}">${img_html}</a>`;
            }
            let icons_html = '<div class="icon platform-icon">';
            for (const platform of platforms) {
                icons_html += PLATFORM_ICONS[platform];
            }
            icons_html += '</div>';
        ...
        title_idx = Array.from(Array(titles.length).keys()).sort((a, b) => titles[a] < titles[b] ? -1 : (titles[b] < titles[a]) | 0);
        rating_idx = Array.from(Array(ratings.length).keys()).sort((a, b) => ratings[b] - ratings[a]);
    ...
    $('#radio-release').prop('checked', true);
    $('#radio-title, #radio-rating').prop('checked', false);

    $('#radio-release').click(function() {
