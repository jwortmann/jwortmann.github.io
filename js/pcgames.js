...
function filterItems() {
    let filter_text = $('#filter').val().toLowerCase();
    if (filter_text) {
    ...
    let filter_tags = filter_text.startsWith('#');
    let status_played = $('#checkbox-status-played').prop('checked');
    let status_unplayed = $('#checkbox-status-unplayed').prop('checked');
    let platform_steam = $('#checkbox-platform-steam').prop('checked');
    let platform_epic = $('#checkbox-platform-epic').prop('checked');
    let platform_ubisoft = $('#checkbox-platform-ubisoft').prop('checked');
    let platform_origin = $('#checkbox-platform-origin').prop('checked');
    let platform_gog = $('#checkbox-platform-gog').prop('checked');
    let platform_retail = $('#checkbox-platform-retail').prop('checked');
    let platform_console = $('#checkbox-platform-console').prop('checked');
    $('#content').children().each(function(index, item) {
        let input = filter_tags && $(item).attr('genre').toLowerCase().includes(filter_text.substring(1)) || !filter_tags && $(item).attr('game-title').toLowerCase().includes(filter_text);
        let played = status_played && $(item).attr('played') == 'true' || status_unplayed && $(item).attr('played') == 'false';
        let platform = (platform_steam && $(item).find(".steam").length ||
                        platform_epic && $(item).find(".epic").length ||
                        platform_ubisoft && $(item).find(".ubisoft").length ||
                        platform_origin && $(item).find(".origin").length ||
                        platform_gog && $(item).find(".gog").length ||
                        platform_retail && $(item).find(".retail").length ||
                        platform_console && $(item).find(".playstation, .playstation-2").length)
        if (input && played && platform) {
            $(this).fadeIn(200);
        } else {
